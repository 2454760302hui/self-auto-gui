...
class TaskManager:
    ...
    async def _execute_agent_stream(
        ...
        enrich_timings: bool = False,
        reset_agent: bool = False,
    ) -> None:
        """Shared agent execution logic for both chat and scheduled workflows."""
        ...
        try:
            with trace_module.trace_context(trace_id):
                ...
                self._register_abort_handler(
                    manager, device_id, cancel_handler, context=context,
                )
            ...
            if abort_registered:
                self._unregister_abort_handler(manager, device_id, context=context)
    ...
    async def _finalize_task(
        ...
        *,
        ...
        try:
            ...
            task_info = await asyncio.to_thread(self.store.get_task, task_id)
            if task_info:
                device_id = task_info.get("device_id", "")
                status_str = "completed" if status == TaskStatus.SUCCEEDED.value else ("failed" if status in (TaskStatus.FAILED.value, "error") else "cancelled")
            ...
            logger.warning(f"Failed to send task completion notification: {exc}")
