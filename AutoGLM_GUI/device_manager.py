...
class ManagedDevice:
    ...
    platform: str = "android"  # "android" or "harmonyos"
...
class DeviceManager:
    ...
    def _poll_devices(self) -> None:
        ...
        # Step 6: Poll HDC (HarmonyOS) devices
        try:
            from AutoGLM_GUI.hdc.connection import list_hdc_devices

            hdc_devices = list_hdc_devices()
            with self._devices_lock:
                for hdc_dev in hdc_devices:
                    serial = f"hdc:{hdc_dev['serial']}"
                    if serial not in self._devices:
                        managed = ManagedDevice(
                            serial=serial,
                            connections=[
                                DeviceConnection(
                                    device_id=hdc_dev["serial"],
                                    connection_type=DeviceConnectionType.HDC,
                                    status=hdc_dev["status"],
                                    last_seen=time.time(),
                                )
                            ],
                            model=hdc_dev.get("model", "HarmonyOS Device"),
                            state=DeviceState.ONLINE if hdc_dev["status"] == "device" else DeviceState.OFFLINE,
                            platform="harmonyos",
                        )
                        self._devices[serial] = managed
                        self._device_id_to_serial[hdc_dev["serial"]] = serial
                        logger.info(f"HDC device added: {serial} ({managed.model})")
                    else:
                        self._devices[serial].last_seen = time.time()
                        self._devices[serial].error_count = 0

                # Mark disconnected HDC devices
                current_hdc_serials = {f"hdc:{d['serial']}" for d in hdc_devices}
                for serial in list(self._devices.keys()):
                    if serial.startswith("hdc:") and serial not in current_hdc_serials:
                        if self._devices[serial].state != DeviceState.DISCONNECTED:
                            self._devices[serial].state = DeviceState.DISCONNECTED
                            logger.warning(f"HDC device disconnected: {serial}")
        except Exception as e:
            logger.debug(f"HDC device polling failed (hdc not available?): {e}")
    ...
    def _ensure_adb_keyboard_once(
        ...
        if managed.connection_type in (DeviceConnectionType.REMOTE, DeviceConnectionType.HDC):
            return
    ...
    def get_device_protocol(self, device_id: ConnectionDeviceID) -> DeviceProtocol:
        ...
        with self._devices_lock:
            ...
            if managed.connection_type == DeviceConnectionType.HDC:
                # HarmonyOS device: 返回 HDCDevice
                local_device_id = managed.primary_device_id
                local_managed = managed

                from AutoGLM_GUI.devices.hdc_device import HDCDevice
                return HDCDevice(local_device_id)
