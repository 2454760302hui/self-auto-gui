...
async def connect_device(sid: str, data: dict[str, Any] | None) -> None:
    ...
    # Check if this is an HDC (HarmonyOS) device — no scrcpy support
    ...
    if managed and managed.connection_type == DeviceConnectionType.HDC:
        await sio.emit(
            "error",
            {
                "message": "HarmonyOS 设备不支持 scrcpy 视频流，请使用截图模式。",
                "type": "hdc_no_scrcpy",
            },
            to=sid,
        )
