...
class DeviceProtocol(Protocol):
    ...
    def paste(self, delay: float | None = None) -> None:
        """
        Paste from clipboard (simulate paste keyevent).

        Args:
            delay: Optional delay after paste in seconds.
        """
        ...

    def select_all(self, delay: float | None = None) -> None:
        """
        Select all (simulate select-all keyevent).

        Args:
            delay: Optional delay after select-all in seconds.
        """
        ...
    ...
    def detect_and_set_adb_keyboard(self) -> str:
        ...
        Note: This is an ADB-specific method. Non-Android implementations
        should use isinstance() checks before calling.
