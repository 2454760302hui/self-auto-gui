...
def list_installed_packages(device_id: str | None = None) -> dict:
    """Query device for installed packages, returning system and third-party lists.
    ...
    sys_result = subprocess.run(
        adb_prefix + ["shell", "pm", "list", "packages", "-s"],
        capture_output=True, text=True,
    )
    system = sorted(
        line.replace("package:", "").strip()
        for line in sys_result.stdout.splitlines()
    ...
    third_party = sorted(
        line.replace("package:", "").strip()
        for line in third_result.stdout.splitlines()
        if line.startswith("package:")
    )
