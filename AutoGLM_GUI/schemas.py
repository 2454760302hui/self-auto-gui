"""Shared Pydantic models for the XNext API."""
...
class KeyEventRequest(BaseModel):
    ...
    @classmethod
    def validate_keycode(cls, v: int) -> int:
        if v < 0 or v > 300:
            raise ValueError("keycode must be between 0 and 300")
        return v
...
# === Direct Device Control Schemas ===


class _ControlDeviceRequest(BaseModel):
    """Base for control requests that need a device_id."""
    device_id: str | None = None


class _DelayMixin(BaseModel):
    delay: float = 0.0
...
class BackRequest(_ControlDeviceRequest, _DelayMixin):
    pass
...
class HomeRequest(_ControlDeviceRequest, _DelayMixin):
    pass
...
class DoubleTapRequest(BaseModel):
    ...
    def validate_coordinates(cls, v: int) -> int:
        if v < 0 or v > 10000:
            raise ValueError("coordinates must be 0-10000")
        return v


class DoubleTapResponse(BaseModel):
    success: bool
    error: str | None = None
...
class LongPressResponse(BaseModel):
    success: bool
    error: str | None = None


class ClearTextRequest(_ControlDeviceRequest):
    pass
...
class ScrollRequest(BaseModel):
    ...
    def validate_distance(cls, v: int) -> int:
        if v < 50 or v > 3000:
            raise ValueError("distance must be 50-3000")
        return v
...
class ScreenshotControlRequest(_ControlDeviceRequest):
    pass


class ScreenshotControlResponse(BaseModel):
    success: bool
    image: str = ""  # base64 encoded PNG
    width: int = 0
    height: int = 0
...
class UiDumpRequest(_ControlDeviceRequest):
    pass
...
class CurrentAppRequest(_ControlDeviceRequest):
    pass
...
class ClipboardGetRequest(_ControlDeviceRequest):
    pass
...
class PasteRequest(_ControlDeviceRequest, _DelayMixin):
    pass
...
class SelectAllRequest(_ControlDeviceRequest, _DelayMixin):
    pass
...
class InstalledAppsRequest(_ControlDeviceRequest):
    pass


class InstalledAppsResponse(BaseModel):
    success: bool
    system: list[InstalledAppInfo] = []
    third_party: list[InstalledAppInfo] = []
    error: str | None = None


class PytestRunRequest(BaseModel):
    code: str
    filename: str = "test_device_control.py"
...
class YamlRunRequest(BaseModel):
    code: str
    filename: str = "test_device.yaml"


class YamlRunResponse(BaseModel):
    success: bool
    output: str = ""
    error: str | None = None


class ForceStopRequest(_ControlDeviceRequest):
    package: str
...
class RecentAppsRequest(_ControlDeviceRequest):
    pass


class RecentAppsResponse(BaseModel):
    success: bool
    packages: list[str] = []  # list of recent package names
    error: str | None = None


class AppStateRequest(_ControlDeviceRequest):
    package: str


class AppStateResponse(BaseModel):
    success: bool
    running: bool  # True if app is currently in foreground / running
    error: str | None = None


class UninstallRequest(_ControlDeviceRequest):
    package: str
...
class GifGenerateRequest(BaseModel):
    images: list[str]  # list of base64-encoded PNG screenshots
    duration_ms: int = 500  # ms per frame


class GifGenerateResponse(BaseModel):
    success: bool
    gif: str = ""  # base64-encoded GIF
    frame_count: int = 0
    error: str | None = None
...
class ConfigResponse(BaseModel):
    ...
    base_url: str
    model_name: str
    api_key: str = ""  # 已废弃，始终返回脱敏值或空字符串
    api_key_masked: str = ""  # 脱敏后的 API Key，前端应使用此字段
    ...
    decision_api_key_masked: str = ""  # 脱敏后的决策模型 API Key

    # 决策模型是否使用了回退（从视觉模型配置继承）
    decision_uses_fallback: bool = False

    # 视觉能力
    supports_vision: bool = True  # 模型是否支持图像输入
...
class ConfigSaveRequest(BaseModel):
    ...
    # 视觉能力
    supports_vision: bool | None = None  # 模型是否支持图像输入
...
# Screen Recording Models


class RecordStartRequest(BaseModel):
    """Start screen recording request."""
    device_id: str | None = None


class RecordStartResponse(BaseModel):
    """Start screen recording response."""
    success: bool
    message: str = ""
    task_id: str = ""
    error: str | None = None
...
class RecordStopRequest(BaseModel):
    """Stop screen recording request."""
    device_id: str | None = None


class RecordStopResponse(BaseModel):
    """Stop screen recording response."""
    success: bool
    message: str = ""
    task_id: str = ""
