...
export class PageAgent {
  ...
  async aiQuery(demand: any, options?: ExecutionTaskProgressOptions) {
    const { output, executor } = await this.taskExecutor.query(demand, options);
    this.appendExecutionDump(executor.dump());
    this.writeOutActionDumps();
  ...
  async aiAssert(
    ...
    const { output, executor } = await this.taskExecutor.assert(assertion, options);
    this.appendExecutionDump(executor.dump());
    this.writeOutActionDumps();
