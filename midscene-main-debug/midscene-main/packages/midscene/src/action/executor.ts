...
export class Executor {
  ...
  onTaskFinish?: ExecutionTaskProgressOptions['onTaskFinish'];

  onTaskError?: ExecutionTaskProgressOptions['onTaskError'];
  ...
  constructor(
    ...
    this.onTaskFinish = options?.onTaskFinish;
      ...
      try {
        ...
        try {
          if (this.onTaskFinish) {
            await this.onTaskFinish(task);
          }
        } catch (e) {
          // console.error('error in onTaskFinish', e);
        }
        taskIndex++;
      } catch (e: any) {
        ...
        try {
          if (this.onTaskError) {
            await this.onTaskError(task);
          }
        } catch (hookError) {
          // console.error('error in onTaskError', hookError);
        }
        break;
      }
    }
