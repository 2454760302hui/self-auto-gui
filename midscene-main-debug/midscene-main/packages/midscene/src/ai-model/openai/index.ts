...
import {
  ...
  ANTHROPIC_BASE_URL,
  ANTHROPIC_DEFAULT_HEADERS_JSON,
  ANTHROPIC_MODEL,
...
export function checkAIConfig(preferVendor?: 'coze' | 'openAI') {
  ...
  if (getAIConfig(ANTHROPIC_AUTH_TOKEN)) return true;
...
export function getModelName() {
  ...
  const anthropicModelName = getAIConfig(ANTHROPIC_MODEL);
  const nameInConfig = getAIConfig(MIDSCENE_MODEL_NAME);
  if (getAIConfig(MIDSCENE_USE_ANTHROPIC_SDK) && anthropicModelName) {
    modelName = anthropicModelName;
  } else if (nameInConfig) {
...
async function createChatClient(): Promise<{
  ...
  if (getAIConfig(MIDSCENE_USE_ANTHROPIC_SDK)) {
    const apiKey =
      getAIConfig(ANTHROPIC_AUTH_TOKEN) || getAIConfig(ANTHROPIC_API_KEY);
    assert(apiKey, 'ANTHROPIC_AUTH_TOKEN or ANTHROPIC_API_KEY is required');
    const anthropicDefaultHeaders = getAIConfigInJson(
      ANTHROPIC_DEFAULT_HEADERS_JSON,
    );
    openai = new Anthropic({
      apiKey,
      baseURL: getAIConfig(ANTHROPIC_BASE_URL),
      defaultHeaders: anthropicDefaultHeaders,
    }) as any;
  }
