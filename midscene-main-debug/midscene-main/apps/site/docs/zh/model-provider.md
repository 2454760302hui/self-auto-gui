...
如果你使用的是 Anthropic 兼容的 relay / proxy 端点，也可以这样配置：
...
# 可选，传递给 Anthropic SDK 初始化的额外请求头
export ANTHROPIC_DEFAULT_HEADERS_JSON='{"x-api-key":"value"}'
```

说明：
- 当你的 relay / proxy 不使用标准 `ANTHROPIC_API_KEY` 认证时，可使用 `ANTHROPIC_AUTH_TOKEN`。
- 当 `MIDSCENE_USE_ANTHROPIC_SDK=1` 时，`ANTHROPIC_MODEL` 的优先级高于 `MIDSCENE_MODEL_NAME`。
- 如果服务商提供的是 OpenAI SDK 兼容接口，则优先使用 `OPENAI_BASE_URL + OPENAI_API_KEY + MIDSCENE_MODEL_NAME`。
