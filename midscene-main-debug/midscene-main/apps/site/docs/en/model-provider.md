...
If you are using an Anthropic-compatible relay or proxy endpoint, you can also configure:

```bash
export MIDSCENE_USE_ANTHROPIC_SDK=1
export ANTHROPIC_BASE_URL="https://your-relay.example/anthropic"
export ANTHROPIC_AUTH_TOKEN="....."
export ANTHROPIC_MODEL="claude-3-5-sonnet-latest"
# optional extra headers for the Anthropic SDK init
export ANTHROPIC_DEFAULT_HEADERS_JSON='{"x-api-key":"value"}'
```

Notes:
- Use `ANTHROPIC_AUTH_TOKEN` for relay/proxy authentication when your endpoint does not use the standard `ANTHROPIC_API_KEY` setup.
- `ANTHROPIC_MODEL` takes precedence over `MIDSCENE_MODEL_NAME` when `MIDSCENE_USE_ANTHROPIC_SDK=1`.
- If your provider exposes an OpenAI-compatible endpoint instead, prefer `OPENAI_BASE_URL + OPENAI_API_KEY + MIDSCENE_MODEL_NAME`.
