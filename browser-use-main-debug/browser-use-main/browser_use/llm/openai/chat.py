	...
	async def ainvoke(
		...
		try:
			...
			if output_format is None:
				...
				# Some models (e.g. GLM thinking mode) put content in reasoning_content when content is None
				msg_content = choice.message.content
				if not msg_content:
					# Try reasoning_content as fallback (GLM-5.1 thinking mode)
					raw_msg = getattr(choice.message, 'model_extra', {}) or {}
					msg_content = raw_msg.get('reasoning_content') or ''
				...
				if choice.message.content is None:
					# Fallback: check reasoning_content (GLM-5.1 thinking mode)
					raw_msg = getattr(choice.message, 'model_extra', {}) or {}
					fallback_content = raw_msg.get('reasoning_content')
					...
					choice.message.content = fallback_content

				usage = self._get_usage(response)

				# Try to parse as JSON; if model returned natural language, extract JSON block
				content_str = choice.message.content or ''
				# Strip markdown code fences
				import re as _re
				json_match = _re.search(r'```(?:json)?\s*([\s\S]+?)\s*```', content_str)
				if json_match:
					content_str = json_match.group(1)
				else:
					# Try to find JSON object in the text
					obj_match = _re.search(r'\{[\s\S]+\}', content_str)
					if obj_match:
