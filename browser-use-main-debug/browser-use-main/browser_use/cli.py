...
def get_llm(config: dict[str, Any]):
	...
	temperature = model_config.get('temperature', llm_config.get('temperature', 0.0))
	...
	if model_name:
		...
		if model_name.startswith('claude'):
			llm_config.setdefault('provider', 'anthropic')
		elif model_name.startswith('gemini'):
			llm_config.setdefault('provider', 'google')
	...
	if llm_config:
		llm_config = dict(llm_config)
		llm_config.setdefault('temperature', temperature)
		return create_llm_from_config(llm_config)

	print(
		'⚠️  No LLM configuration found. Please update your config or set one of: OPENAI_API_KEY, ANTHROPIC_API_KEY, ANTHROPIC_AUTH_TOKEN, or GOOGLE_API_KEY.'
	)
	sys.exit(1)
