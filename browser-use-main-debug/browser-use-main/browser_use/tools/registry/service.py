...
class Registry(Generic[Context]):
	...
	def _replace_sensitive_data(
		...
		"""
		# Compile pattern once - only match valid identifier-like placeholder names
		# to prevent regex injection via crafted placeholder names
		secret_pattern = re.compile(r'<secret>([\w\-\.@]+)</secret>')
		...
		applicable_secrets: dict[str, str] = {}
		...
		def _get_replacement(placeholder: str) -> str | None:
			"""Get the replacement value for a placeholder, generating TOTP if needed."""
			if placeholder not in applicable_secrets:
				return None
		...
		def recursively_replace_secrets(value: str | dict | list) -> str | dict | list:
			if isinstance(value, str):
				...
				for placeholder in matches:
					replacement_value = _get_replacement(placeholder)
				...
				if value in applicable_secrets:
					replacement_value = _get_replacement(value)
					if replacement_value is not None:
						replaced_placeholders.add(value)
		...
		if all_missing_placeholders:
			logger.warning(f'Missing or empty keys in sensitive_data dictionary: {", ".join(sorted(all_missing_placeholders))}')
