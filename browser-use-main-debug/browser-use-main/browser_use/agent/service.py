...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	The Agent class orchestrates browser automation tasks using language models.
	It manages browser sessions, executes actions, and maintains conversation history.

	Args:
		task: The task description for the agent to accomplish
		llm: Language model instance (defaults to ChatBrowserUse if not provided)

		# Browser Configuration
		browser_profile: Browser configuration profile
		browser_session: Existing browser session to reuse
		browser: Alias for browser_session (preferred)

		# Tools & Actions
		tools: Custom tools/actions registry
		controller: Alias for tools (preferred)

		# Skills Integration
		skill_ids: List of skill IDs to enable (or ['*'] for all)
		skills: Alias for skill_ids (preferred)
		skill_service: Pre-configured skill service instance

		# Initial Execution
		sensitive_data: Credentials/secrets for form filling (format: {key: value} or {domain: {key: value}})
		initial_actions: Actions to execute before starting the task

		# Callbacks
		register_new_step_callback: Called after each agent step
		register_done_callback: Called when task completes
		register_external_agent_status_raise_error_callback: External error check
		register_should_stop_callback: External stop condition check

		# Agent Behavior
		output_model_schema: Pydantic model for structured output
		extraction_schema: JSON schema for data extraction
		use_vision: Enable vision capabilities (True/False/'auto')
		...
		flash_mode: Simplified mode for faster execution
		demo_mode: Enable in-browser demo panel
		max_history_items: Maximum conversation history items to keep
		page_extraction_llm: LLM for page content extraction
		fallback_llm: Fallback LLM if primary fails
		use_judge: Enable task completion judging
		ground_truth: Expected outcome for evaluation
		judge_llm: LLM for judging task completion
		llm_timeout: Timeout for LLM requests (seconds)
		step_timeout: Timeout for each step (seconds, default: 180)
		directly_open_url: Auto-navigate to URLs found in task
		include_recent_events: Include browser events in context
		final_response_after_failure: Generate response even on failure
		enable_planning: Enable multi-step planning
		planning_replan_on_stall: Steps before replanning (default: 3)
		planning_exploration_limit: Max exploration steps (default: 5)
		loop_detection_window: Window size for loop detection (default: 20)
		loop_detection_enabled: Enable loop detection
		llm_screenshot_size: Resize screenshots for LLM (width, height)
		message_compaction: Enable/configure message compaction
		max_clickable_elements_length: Max length of clickable elements list

		# File System
		generate_gif: Generate GIF of agent execution
		available_file_paths: List of file paths accessible to agent
		file_system_path: Base directory for agent file system
		display_files_in_done_text: Show file list in completion message

		# Advanced
		include_attributes: HTML attributes to include in DOM
		include_tool_call_examples: Include examples in tool descriptions
		vision_detail_level: Vision detail level ('auto'/'low'/'high')
		sample_images: Sample images for vision tasks
		calculate_cost: Calculate and track token costs
		pricing_url: URL for model pricing data
		injected_agent_state: Pre-initialized agent state
		source: Source identifier for telemetry
		task_id: Custom task ID (auto-generated if not provided)
		enable_signal_handler: Enable SIGINT/SIGTERM handlers
