...
def _log_format_message_line(message: BaseMessage, content: str, is_last_message: bool, terminal_width: int) -> list[str]:
	...
	try:
		...
		# Token count is not directly available on the message object.
		# Token tracking requires access to the LLM response (ChatInvokeCompletion.usage),
		# which is not available in the message logging context.
		# We show a placeholder that indicates token tracking is handled separately.
		token_str = '    '
...
class MessageManager:
	...
	def _filter_sensitive_data(self, message: BaseMessage) -> BaseMessage:
		"""Filter out sensitive data from the message, replacing real values with placeholder tags."""

		def replace_sensitive(value: str) -> str:
			if not self.sensitive_data:
				return value

			# Collect all sensitive values, converting old format to new format
			...
			# Sort by value length descending to replace longer values first (avoids partial replacements)
