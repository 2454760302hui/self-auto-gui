...
async def handle(action: str, session: SessionInfo, params: dict[str, Any]) -> Any:
	...
	if action == 'open':
		...
		# Ensure URL has scheme, but preserve special schemes like data:
		if '://' not in url and not url.startswith(('data:', 'file:', 'about:', 'javascript:')):
			url = 'https://' + url

		from browser_use.browser.events import NavigateToUrlEvent

		await bs.event_bus.dispatch(NavigateToUrlEvent(url=url, new_tab=new_tab))
