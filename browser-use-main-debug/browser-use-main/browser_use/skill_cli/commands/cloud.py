...
def _base_url(version: str) -> str:
	...
	return os.environ.get(env_key, f'{CONFIG.BROWSER_USE_CLOUD_API_URL.rstrip("/")}/api/{version}')


def _spec_url(version: str) -> str:
	env_key = f'BROWSER_USE_OPENAPI_SPEC_URL_{version.upper()}'
	return os.environ.get(env_key, f'{CONFIG.BROWSER_USE_CLOUD_API_URL.rstrip("/")}/api/{version}/openapi.json')
