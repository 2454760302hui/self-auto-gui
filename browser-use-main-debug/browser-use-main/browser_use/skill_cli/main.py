...
def _connect_to_daemon(timeout: float = 180.0, session: str = 'default') -> socket.socket:
...
def ensure_daemon(
	...
	*,
	...
	for _ in range(600):  # 30 seconds max
		if _is_daemon_alive(session):
			return
		time.sleep(0.05)
...
Setup:
  ...
  browser-use init                              # Generate template file""")
	...
	p = subparsers.add_parser('serve', help='Run HTTP service for browser control')
	p.add_argument('--host', default='127.0.0.1', help='Host to bind HTTP server')
	p.add_argument('--port', type=int, default=9242, help='Port to bind HTTP server')
...
def main() -> int:
	...
	if args.command == 'serve':
		from browser_use.skill_cli.http_server import serve
