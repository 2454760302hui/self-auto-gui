...
class BrowserWrapper:
	...
	def extract(self, query: str) -> Any:
		...
		Note: This feature requires LLM integration. For full extraction capabilities,
		use agent.run() with the extract action instead.
		"""
		import logging
		logger = logging.getLogger(__name__)
		logger.warning(
			'extract() is not available in CLI mode. '
			'Use agent.run() with the extract action for data extraction.'
		)
		return None
