	...
	async def test_generate_gif(self, test_dir, httpserver_url, llm, generate_gif):
		...
		try:
			...
			if generate_gif is False:
				# No GIF should be created when disabled
				...
				# Default path: agent_history.gif
				default_gif = Path('agent_history.gif')
				# GIF may or may not be created depending on screenshot availability
				# Check that if created, it's valid
				if default_gif.exists():
					assert default_gif.stat().st_size > 0, 'GIF file is empty'
			else:  # custom_path
				# Custom path: file should be at custom location if screenshots available
				if expected_gif_path and expected_gif_path.exists():
					assert expected_gif_path.stat().st_size > 0, 'Custom GIF file is empty'
		finally:
			await browser_session.kill()
