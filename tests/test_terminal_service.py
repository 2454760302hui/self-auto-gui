...
def test_build_terminal_environment_includes_project_tools(
    ...
    # Create Unix-style venv layout (matching the code's is_windows() check)
    ...
    # Ensure _build_terminal_environment sees a Unix venv layout
...
async def test_terminal_output_limit_triggers_close() -> None:
    ...
    # Patch _publish_output to avoid asyncio.create_task issues in trio mode.
    # In real asyncio code, this would use asyncio.create_task; in trio, that fails.
    async def patched_publish(self, stream: str, chunk: bytes) -> bool:
        ...
        await self._publish(
            {"type": "output", "stream": stream, "data": chunk.decode("utf-8", errors="replace")}
        )
        return True

    import types

    session._publish_output = types.MethodType(patched_publish, session)
...
async def test_start_posix_closes_pty_fds_when_spawn_fails(
    ...
    pty = pytest.importorskip("pty")  # Skip on Windows where pty/termios are unavailable
