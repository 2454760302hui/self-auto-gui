...
export function DevicePanel({
  ...
  return (
    <div className="flex-1 flex gap-4 p-4 items-stretch justify-center min-h-0">
      {/* Chat area - NEXA clean SaaS style */}
      <Card className="flex-1 flex flex-col min-h-0 max-w-2xl">
        {/* Header - NEXA clean style */}
          ...
          >
            {messages.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center min-h-[calc(100%-1rem)] animate-fade-in">
                {/* NEXA Welcome - Clean Modern Design */}
                <div className="mb-6">
                ...
                <p className="text-sm text-muted-foreground max-w-xs">
                  {t.devicePanel.describeTask || '描述你的任务，AI 将自动操控手机完成'}
                </p>
                {/* Quick actions */}
                <div className="flex flex-wrap gap-2 mt-6 justify-center max-w-sm">
                  {['打开微信', '截图保存', '检查消息', '打开设置'].map(action => (
                    <button
                      key={action}
                      className="px-3 py-1.5 rounded-full text-xs border border-border hover:bg-secondary/50 transition-all"
                      onClick={() => {
                        const textarea = document.querySelector<HTMLTextAreaElement>('textarea[placeholder*="任务"]');
                        if (textarea) {
                ...
                >
                  {message.role === 'assistant' ? (
                    <div className="max-w-[85%] space-y-3">
                      ...
                      {/* Current thinking being streamed - NEXA clean style */}
