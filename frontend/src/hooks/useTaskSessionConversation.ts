...
export function useTaskSessionConversation({
  ...
  const capMessages = (msgs: TaskConversationMessage[]) =>
  ...
  const replaceTaskMessages = useCallback((taskId: string) => {
    ...
    setMessages(previousMessages => {
      ...
      let result: TaskConversationMessage[];
      if (userIndex === -1 || assistantIndex === -1) {
        result = [...previousMessages, ...pair];
      } else {
        result = previousMessages.map(message => {
          if (message.id === pair[0].id) return pair[0];
          if (message.id === pair[1].id) return pair[1];
          return message;
        });
      }
      return capMessages(result);
    });
  }, []);
